...
def fft_trans(img_arr):
    fft_list = torch.fft.fftshift(fft.fft2(torch.reshape(img_arr, (-1, img_arr.shape[2], img_arr.shape[3])).to(torch.complex64)))
    re_list, im_list = fft_list.real, fft_list.imag
    f2_list = torch.square(re_list) + torch.square(im_list)
    return torch.log10(torch.clamp(f2_list, min=1e-36, max=1e36)).unsqueeze(1)
...
class DRIT(nn.Module):
  ...
  def test_forward_transfer(self, image_a, image_b, a2b=True):
    ...
    return self.normalize_image(output)

  def test_forward_step(self, image_a, image_b, a2b=True):
    self.z_content_a, self.z_content_b = self.enc_c.forward(image_a, image_b)
    if self.concat:
      self.mu_a, self.logvar_a, self.mu_b, self.logvar_b = self.enc_a.forward(image_a, image_b)
      std_a = self.logvar_a.mul(0.5).exp_()
    ...
    if a2b:
      ...
      output = self.gen.forward_a(self.z_content_b, self.z_attr_a)
    return self.z_content_a, self.z_content_b, self.z_attr_a, self.z_attr_b, self.normalize_image(output)
  ...
  def update_EG(self):
    ...
    # update G, Ec
    self.enc_c_opt.zero_grad()
    self.enc_a_opt.zero_grad() # add
    self.gen_opt.zero_grad()
    self.backward_G_alone()
    self.enc_c_opt.step()
    self.enc_a_opt.step() # add
    self.gen_opt.step()
  ...
  def backward_EG(self):
    ...
    # loss fakeA to B or fakeB to A
    loss_G_L1_AB = self.criterionL1(self.real_A_encoded, self.fake_B_encoded) * 0.5
    loss_G_L1_BA = self.criterionL1(self.real_B_encoded, self.fake_A_encoded) * 0.5
  ...
  def backward_G_alone(self):
    ...
    with torch.autograd.detect_anomaly():
      # loss_z_L1.backward()
      loss_z_L1_detach = loss_z_L1.detach_().requires_grad_(True)
      loss_z_L1_detach.backward()
  ...
  def aim_output(self):
    images_b1 = self.normalize_image(self.fake_B_encoded).detach()
    return images_b1[0:1, ::]

  def init_gen(self, image_a, image_b):
    self.input_A = image_a
    self.input_B = image_b
    self.forward()
