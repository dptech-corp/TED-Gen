...
def to_database(database_path = '', dat_path = ''):
    ...
    with open(dat_path, 'r') as data_paths:
        ...
        for i in range(len(data_path)):
            label_n = Image.open(data_path[i][:-1] + '/ReS2.png')
...
def generate_data(structure_path='',DAT_path = '',move_list = None,incostem_path = './incostem',image_size = 4096,image_num = 1, no_show_s = True):
    ...
    for i in range(len(ReS2_layer1)):
        if random.uniform(0,1)>0.02:
            ReS2_layer1.positions[i,:2] += (random.uniform(-0.1,0.1),random.uniform(-0.1,0.1))

        if random.uniform(0,1)>0.02:
            ReS2_layer2.positions[i,:2] += (random.uniform(-0.1,0.1),random.uniform(-0.1,0.1))

    if random.uniform(0,1)>2.0:
        r = random.uniform(0.,360.)
    else: r = 0
    # center = ReS2_layer2[650].position
    ...
    path = './data/ReS2_'+str(move_x)[:6]+'_'+str(move_y)[:6]+'_'+str(r)[:4]+'_'+if_flip
    os.makedirs(path)

    if not os.path.exists('./dat'):
        os.makedirs('./dat')

    write_xyz(ReS2,file_name=path+'/ReS2.xyz', no_show_s = no_show_s)
    with open(DAT_path,'a') as dat:
        dat.write(path+'\n')
        

    ReS2_layer1 = ReS2[:len(ReS2_layer1)]
    write_xyz(ReS2_layer1,file_name=path+'/ReS2_layer1.xyz')
    # save_label(image_size,ReS2_layer1,file_name=path+'/ReS2_layer1')

    ReS2_layer2 = ReS2[len(ReS2_layer1):]
    write_xyz(ReS2_layer2,file_name=path+'/ReS2_layer2.xyz')
    # save_label(image_size,ReS2_layer2,file_name=path+'/ReS2_layer2')

    save_train_label(image_size,ReS2_layer1,ReS2_layer2,file_name=path+'/ReS2')
...
for x in range(-33,33,4):
    for y in range(0,28,4):
        generate_data(structure_path='./structure/ReS2/one_layer.xyz',
                      DAT_path = './dat/ReS2.dat',
                      move_list=[x/10.,y/10.],
                      image_size = 2048,
                      image_num=1, 
                      no_show_s = False)

to_database(database_path='./database',dat_path='./dat/ReS2.dat')