...
def to_database(database_path = '', dat_path = ''):
    ...
    with open(dat_path, 'r') as data_paths:
        ...
        for i in range(len(data_path)):
            label_n = Image.open(data_path[i][:-1] + '/MoS2.png')
            for img in os.listdir(data_path[i][:-1]+'/image'):
                image = Image.open(data_path[i][:-1]+'/image/'+img)
                image = TF.crop(image, image.size[0]//2 - 900, image.size[1]//2, 1024, 1024)
                label = TF.crop(label_n.copy(), label_n.size[0]//2 - 900, label_n.size[1]//2, 1024, 1024)
...
def generate_data(structure_path='',DAT_path = '',move_list = None,incostem_path = './incostem',image_size = 4096,image_num = 1, no_show_s = True):
    ...
    MoS2_layer1 = atoms.copy()
    MoS2_layer2 = atoms.copy()

    MoS2_layer2.positions[:,2] += 7.

    if random.uniform(0,1)>2.0:
        r = random.uniform(0.,180.)
    else: r = move_list[2]
    center = atoms.get_cell().sum(axis=0) / 2.0
    MoS2_layer2.rotate(r,'z',center)
    ...
    MoS2_layer2.positions[:,0] += move_x
    MoS2_layer2.positions[:,1] += move_y
    MoS2_layer2.positions[:,2] += move_z

    if random.uniform(0,1)>2.0:
        if_flip = '1'
        MoS2_layer2.rotate(180,'y',center)
    else:if_flip = '0'

    # for i in range(image_num):
    #     MoS2_layer1.pop(random.randint(0,len(MoS2_layer1)-10))
    #     MoS2_layer2.pop(random.randint(0,len(MoS2_layer2)-10))

    MoS2 = MoS2_layer1 +MoS2_layer2
    # MoS2.center(vacuum=1)

    MoS2_layer2 = MoS2[len(MoS2_layer1):]
    MoS2_layer1 = MoS2[:len(MoS2_layer1)]

    path = './data/MoS2_'+str(move_x)[:6]+'_'+str(move_y)[:6]+'_'+str(r)[:4]+'_'+if_flip
    os.makedirs(path)

    if not os.path.exists('./dat'):
        os.makedirs('./dat')

    write_xyz(MoS2,file_name=path+'/MoS2.xyz', no_show_s = no_show_s)
    # save_label(image_size,MoS2,file_name=path+'/MoS2')
    with open(DAT_path,'a') as dat:
        dat.write(path+'\n')
        

    MoS2_layer1 = MoS2[:len(MoS2_layer1)]
    write_xyz(MoS2_layer1,file_name=path+'/MoS2_layer1.xyz')
    # save_label(image_size,MoS2_layer1,file_name=path+'/MoS2_layer1')

    MoS2_layer2 = MoS2[len(MoS2_layer1):]
    write_xyz(MoS2_layer2,file_name=path+'/MoS2_layer2.xyz')
    # save_label(image_size,MoS2_layer2,file_name=path+'/MoS2_layer2')

    save_train_label(image_size,MoS2_layer1,MoS2_layer2,file_name=path+'/MoS2')
...
r=10
for x in range(-18,18,2):
    for y in range(0,14,2):
        generate_data(structure_path='./structure/MoS2/one_layer.xyz',
                          DAT_path = './dat/MoS2.dat',
                          move_list=[x/10.,y/10.,r/10.],
                          image_size = 2048,
                          image_num=1, 
                          no_show_s = True)


to_database(database_path='',
            dat_path='./dat/MoS2.dat')